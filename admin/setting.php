<?php
  ...
  $sql_movie1 = mysqli_query($sql_con, 'SELECT * FROM movie ORDER BY no DESC') or die();
  $sql_movie2 = mysqli_query($sql_con, 'SELECT * FROM movie ORDER BY no DESC') or die();
  $sql_movie3 = mysqli_query($sql_con, 'SELECT * FROM movie ORDER BY no DESC') or die();
  $sql_movie4 = mysqli_query($sql_con, 'SELECT * FROM movie ORDER BY no DESC') or die();
  ...
  $setting = mysqli_fetch_array($sql_setting);
    ...
    `slide3_img` = '".$_POST["slide3_img"]."', 
		`movie_hot1` = '".$_POST["movie_hot1"]."', 
    `movie_hot2` = '".$_POST["movie_hot2"]."', 
    `movie_hot3` = '".$_POST["movie_hot3"]."', 
		`movie_hot4` = '".$_POST["movie_hot4"]."'");
  ...
  <div id="wrapper">
    ...
    <div id="content-wrapper">
      ...
      <div class="container-fluid">
        ...
        <hr>
        <h4><i class="fas fa-video"></i> ตั้งค่า หนังแนะนำ</h4>
        <div class="col-12 col-md-12 col-lg-12">
          <div class="form-inline">
          <label style="font-weight: bold">เรื่องที่ 1</label>&nbsp;&nbsp;&nbsp;&nbsp;
            <select name="movie_hot1" class="form-control">
              <?php if($setting["movie_hot1"]==0) {?><option value="0" selected>=== กรุณาเลือก ===</option><?php }?>
              <option value="0">------ ปิดการใช้งาน ------</option>
              <?php while($movie_hot1 = mysqli_fetch_array($sql_movie1)) { ?>
                <option value="<?=$movie_hot1["no"]?>" <?php if($setting["movie_hot1"]==$movie_hot1["no"]) { echo "selected"; }?>><?=$movie_hot1["name"]?></option>
              <?php } ?>
            </select>
          </div>
          <div class="form-inline">
          <label style="font-weight: bold">เรื่องที่ 2</label>&nbsp;&nbsp;&nbsp;&nbsp;
            <select name="movie_hot2" class="form-control">
              <?php if($setting["movie_hot2"]==0) {?><option value="0" selected>=== กรุณาเลือก ===</option><?php } ?>
              <option value="0">------ ปิดการใช้งาน ------</option>
              <?php while($movie_hot2 = mysqli_fetch_array($sql_movie2)) { ?>
                <option value="<?=$movie_hot2["no"]?>"  <?php if($setting["movie_hot2"]==$movie_hot2["no"]) { echo "selected"; }?>><?=$movie_hot2["name"]?></option>
              <?php } ?>
            </select>
          </div>
          <div class="form-inline">
          <label style="font-weight: bold">เรื่องที่ 3</label>&nbsp;&nbsp;&nbsp;&nbsp;
            <select name="movie_hot3" class="form-control">
              <?php if($setting["movie_hot3"]==0) {?><option value="0" selected>=== กรุณาเลือก ===</option><?php } ?>
              <option value="0">------ ปิดการใช้งาน ------</option>
              <?php while($movie_hot3 = mysqli_fetch_array($sql_movie3)) { ?>
                <option value="<?=$movie_hot3["no"]?>"  <?php if($setting["movie_hot3"]==$movie_hot3["no"]) { echo "selected"; }?>><?=$movie_hot3["name"]?></option>
              <?php } ?>
            </select>
          </div>
          <div class="form-inline">
          <label style="font-weight: bold">เรื่องที่ 4</label>&nbsp;&nbsp;&nbsp;&nbsp;
            <select name="movie_hot4" class="form-control">
              <?php if($setting["movie_hot4"]==0) {?><option value="0" selected>=== กรุณาเลือก ===</option><?php } ?>
              <option value="0">------ ปิดการใช้งาน ------</option>
              <?php while($movie_hot4 = mysqli_fetch_array($sql_movie4)) { ?>
                <option value="<?=$movie_hot4["no"]?>"  <?php if($setting["movie_hot4"]==$movie_hot4["no"]) { echo "selected"; }?>><?=$movie_hot4["name"]?></option>
              <?php } ?>
            </select>
          </div>
        </div>
